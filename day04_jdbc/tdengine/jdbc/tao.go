...
func main2() {
	var taosuri = "root:taosdata@/tcp(172.31.0.220:6030)/"
	...
	_, err = taos.Exec("create database if not exists wsl")
	...
	rows, err := taos.Query("select * from tb")
	if err != nil {
	...
	for rows.Next() {
		...
		err := rows.Scan(&r.ts, &r.a, &r.b)
		if err != nil {
			fmt.Println("scan error:\n", err)
			return
		}
		fmt.Println(r.ts, r.a, r.b)
	}
}
